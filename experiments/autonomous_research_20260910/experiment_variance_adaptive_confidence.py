...
"""Experiment: Variance-Adaptive Confidence (VAC) for C-FlowBalance.

Investigates whether dynamically scaling teacher confidence alpha based on group
reward standard deviation improves exploration and convergence:
    alpha_adaptive(x) = alpha_min + (alpha_max - alpha_min) * exp(-std(R) / sigma_0)
"""
...
def compute_vac_advantages(
    ...
    std_R = scores.std().item()

    if vac_enabled:
        # Variance-Adaptive Confidence: scale alpha higher when group has zero/low reward variance
        alpha = alpha_min + (alpha_max - alpha_min) * float(np.exp(-std_R / max(sigma_0, 1e-4)))
    else:
        alpha = alpha_base
    ...
    A_SubTB = 0.5 * A_DB + 0.5 * A_TB
    return A_SubTB, alpha


def run_vac_benchmark():
    configs = [
        ("Fixed_Alpha_0.2", False, 0.2),
        ("Fixed_Alpha_0.5", False, 0.5),
        ("Fixed_Alpha_0.8", False, 0.8),
        ("Adaptive_VAC", True, 0.5),
    ]
    ...
    for name, vac_enabled, alpha_val in configs:
        print(f"\nEvaluating: {name} ...")
        ...
        for ep in range(epochs):
            ...
            for p_idx in range(num_problems):
                ...
                adv, eff_alpha = compute_vac_advantages(
            ...
            mean_alpha = float(np.mean(ep_alphas))

            acc_curve.append(pass1)
            hard_acc_curve.append(hard_pass1)
            effective_alphas.append(mean_alpha)

            if (ep + 1) % 5 == 0 or ep == epochs - 1:
                print(f"  Epoch {ep+1:02d}: Pass@1 = {pass1*100:.1f}%, Hard Pass@1 = {hard_acc_curve[-1]*100:.1f}%, Mean Alpha = {mean_alpha:.3f}")

        results[name] = {
            "final_pass1": acc_curve[-1],
            "final_hard_pass1": hard_acc_curve[-1],
            "final_alpha": effective_alphas[-1],
            "acc_curve": acc_curve,
            "hard_acc_curve": hard_acc_curve,
        }

    out_file = "experiments/autonomous_research_20260910/vac_benchmark_results.json"
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 80)
    print(" VARIANCE-ADAPTIVE CONFIDENCE (VAC) BENCHMARK REPORT")
    print("=" * 80)
    print(f"{'Configuration':<24} | {'Pass@1 (%)':<14} | {'Hard Pass@1 (%)':<18} | {'Final Alpha':<12}")
    print("-" * 80)
    for name, r in results.items():
        print(f"{name:<24} | {r['final_pass1']*100:<14.2f} | {r['final_hard_pass1']*100:<18.2f} | {r['final_alpha']:<12.3f}")
    print("=" * 80)
