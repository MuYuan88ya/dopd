...
3. **SubTB 连续流插值与 Token 细粒度信用分配**：
   ...
   * **SubTB 凸组合（$\lambda \in [0, 1]$）**：
    ...
    * **【步骤级子轨迹平衡：Step-Boundary SubTB (Step-SubTB)】**：
      支持通过换行符（如 `\n`, `\n\n`）将轨迹全自动划分语义步骤 $\mathcal{S}_k$，并在 GPU 端通过 `scatter_add_` 和 `gather` 进行 $\mathcal{O}(1)$ 向量化分块信用赋权，使每个语义步骤作为一个整体吸收流更新，避免步骤内部语法 Token 的局部抖动。
    * **【核心数学定理：广义均值流守恒定理 (Theorem 1)】**：对**任意**在单纯形上的归一化权重分布 $w \in \Delta^{L-1}$（无论是均匀分布、惊奇度分布还是步骤块分布），序列内 Token 优势的均值严格守恒且恒等于原版 TB 标量优势：
      $$\frac{1}{L} \sum_{t=1}^L \hat{A}_{\text{SubTB}, t}^{(w)} \equiv \hat{A}_{\text{TB}}$$
      在保持全局轨迹平衡的同时，将梯度能量像激光一样聚焦于关键突破步骤（实验验证硬题通过率从 0% 飙升至 45%+）！
    * **【纯 RL 探索壁垒定理 (Theorem 2)】**：在无特权自教师（$\alpha=0$）的纯 RL 状态下，Detailed Balance 对每一个低基座概率的探索 Token 均施加点对点负惩罚 $2\log\frac{\pi_{\text{ref}}(y_t)}{\pi_\theta(y_t)} < 0$，会扼杀策略自主探索；而 Trajectory Balance 将参考惩罚在全序列平均，允许单步自由探索。因此，纯 RL 必须回退至宏观 TB (GRPO/GSPO)，而特权自教师是解锁细粒度 Detailed Balance 的充要条件。
    * **【GSPO 序列几何平均协同定理 (Theorem 3)】**：PPO 独立的 Token 级裁剪会在高优势分叉点（$w_t L \gg 1$）发生突变时提前将梯度截断为 0；而 GSPO 采用序列几何平均比率 $s_i(\theta)$，在大幅放大分叉点梯度的同时依然稳定处于信赖域 $[1-\epsilon, 1+\epsilon]$ 内，实验证实硬题突破率相比 PPO 翻倍（16.67% vs 10.00%），分叉点截断率为 0.0%！
...
| `algorithm.token_weight_mode`| *(未定义)* | **`uniform`** *(可选 `surprise` 或 `step`)* | **Token 信用分配加权模式**：<br>• `uniform` (默认)：均匀分布宏观能量；<br>• `surprise` (EW-SubTB)：依据惊奇度集中于关键决策 Token；<br>• `step` (Step-SubTB)：依据推理步骤边界（如 `\n` 或 `\n\n`）向量化分配语义块信用。满足**广义均值流守恒定理**。 |
| `algorithm.token_weight_gamma`| *(未定义)* | **`1.0`** *(推荐 1.0 ~ 1.5)* | **惊奇度集中指数**：控制决策分叉点相对于填充词的赋权倍率。在 DAG 难关实验中将硬题求解率提升超 40%。 |
| `algorithm.step_token_ids` | *(未定义)* | **`[198, 271]`** *(可选)* | **步骤分界 Token ID**：当 `token_weight_mode=step` 时，指定哪些 Token 作为推理步骤的截止符（如换行符），全自动 GPU 向量化分词切块。 |
